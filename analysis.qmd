...
source("R/behavior_distribution.R")
source("R/behavior_distribution_average.R")
...
```{r}
p2 <- reward_plots(preprocessed_df, experiment_name, version, save = TRUE)
print(p2)
```
...
```{r}
p3 <- entropy_plots(preprocessed_df, experiment_name, version, save = TRUE)
print(p3)
```
...
behavior_distribution(preprocessed_df, experiment_name, version, save = TRUE)
```
...
p_dist_avg <- behavior_distribution_average(preprocessed_df, experiment_name, version, save = TRUE)
print(p_dist_avg)Round Y
```
...
```{r}
sort(c(0.55,0,0.8,0.5,0.4,0.3,0.6,0.75,0.1,0.35,0.45,0.2))
```